...
type TVerifyState = {
  key: string, verifyRequest: boolean, verifyFailed: boolean,
}

const initialVerifyState: TVerifyState = {
  key: '', verifyRequest: false, verifyFailed: false,
};

export const verifyReducer = (state = initialVerifyState, action: TVerifyActions) => {
  switch (action.type) {
    ...
    case SET_VERIFY_SUCCESS: {
      return {
        ...state, key: action, verifyRequest: false,
      };
    }
